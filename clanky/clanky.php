...
$dotaz=mysql_query("SELECT * FROM katclan where id='$a'");
while ($vypsat=mysql_fetch_assoc($dotaz))
{$kat1 = "".$vypsat['nazev']."";}
ocp(''.$jazyk['web_449'].': '.$kat1.'');
$vyber55=mysql_query("select * from clanky where kat='$a'");
$pocet55 = mysql_num_rows($vyber55);
if ($pocet55=='0') {
echo "<p align='center'><br /><span style='color: red;'>".$jazyk['web_450']."</span></p>";
}
$PZS = $settings2['stahovanie']; 
$str = 1;  
if(isset($_GET["str"]) && $_GET["str"] > 0) {$str = $_GET["str"];} 
$start = ($str - 1) * $PZS;
$query = "SELECT * FROM clanky where kat='$a' ORDER BY id DESC ";
$pocetVysledku = mysql_num_rows(mysql_query($query)); 
$query .= " LIMIT $PZS OFFSET $start";
$mysql = mysql_query($query);
while ($vypsat=mysql_fetch_assoc($mysql))
{
echo "<br /><table align='center' class='gal_clan'><tr><td><p align='center'><strong><a href='clanek.php?id=".$vypsat['id']."'>".$vypsat['nazev']."</a></strong></p>".$vypsat['popis']."
<table width='100%'><tr>
